    ...
    class Response < SamlMessage
      ...
      def validate_subject_confirmation
        ...
        subject_confirmation_nodes.each do |subject_confirmation|
          if subject_confirmation.attributes.include?("Method") && subject_confirmation.attributes['Method'] != 'urn:oasis:names:tc:SAML:2.0:cm:bearer'
            next
          end
          ...
          next if (attrs.include?("InResponseTo") && attrs['InResponseTo'] != in_response_to) ||
                  (attrs.include?("NotOnOrAfter") && (parse_time(confirmation_data_node, "NotOnOrAfter") + allowed_clock_drift) <= now) ||
